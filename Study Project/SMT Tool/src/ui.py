...
class UI(object):
    ...
    def menu(self):
        ...
        menu_main.add_command(label="关于", command=lambda: self.show_frame(self.about_frame))

        # 页面:二级菜单
        menu_page.add_cascade(label="主页", command=lambda: self.show_frame(self.home_frame))
        menu_page.add_cascade(label="日程", command=lambda: self.show_frame(self.schedule_frame))
        menu_page.add_cascade(label="设置", command=lambda: self.show_frame(self.setting_frame))
        menu_page.add_cascade(label="关于", command=lambda: self.show_frame(self.about_frame))

        # 新建:二级菜单
        menu_new.add_cascade(label="新建日程", command=lambda: self.pop_up.information("em...\n还没写，要不你先看看别的？"))
    ...
    def home(self):
        ...
        # 测试代码
        label = tkinter.Label(self.home_frame, text="这是主页", font=("Arial", 11))
